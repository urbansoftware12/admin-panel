...
export default function DefaultOrPic(props) {
    if(!props.src) return (
        <div className={`w-full bg-gray-100 h-full flex ${props.mega?"flex-col items-center":"items-end"} justify-center`} >
            ...
            <p className={`${props.mega?null: "hidden"} mt-4 lg:mt-7 font_furura text-center text-lg lg:text-xl`}>Drop Image here.<br /> .webp format image is highly recommended</p>
        </div>
    )
