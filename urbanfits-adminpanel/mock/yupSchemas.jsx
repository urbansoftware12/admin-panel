...
export const addProductSchema = Yup.object().shape({
    ...
    variants: Yup.array().of(
        Yup.object().shape({
            ...
            images: Yup.array().of(Yup.string().required('Image is required. ')).min(1, 'minimum 6 images are required (if not available, then repeat images).').max(6, 'You can only upload maximum 6 images.'),
            sizes: Yup.array().of(
                Yup.object().shape({
                    size: Yup.string(),
                    quantity: Yup.number()
